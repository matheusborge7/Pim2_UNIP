...
/* Helpers */
// implementa strdup caso não exista (safe)
...
/* Carrega usuários do arquivo para o array já alocado. Se exceder MAX_USERS, ignora o restante. */
void carregar_usuarios() {
    ...
    while (fgets(linha, sizeof(linha), f)) {
        ...
        if (!copy) { printf("Erro de memória ao processar linha.\n"); break; }
...
/* Salva até usuarios_count usuários no arquivo */
...
/* Inicializa arrays de tentativas (alocação única). Chamar no inicio do programa. */
int init_tentativas_arrays() {
    if (tentativa_emails) return 1; /* já inicializado */
...
/* ---------- Funções de manipulação das strings compostas ---------- */
...
void adicionar_curso_em_usuario(Usuario *al, const char *curso) {
    ...
    if (count >= 3) { printf("Aluno já possui 3 cursos.\n"); return; }
...
/* ---------- Login / Cadastro (login por índice) ---------- */

/* login retorna índice do usuário encontrado (>=0) e copia email em out_email (se não NULL). -1 em falha */
...
void menu_professor(const char *email_prof) {
    ...
    trim(opcao);
        if (strcmp(opcao, "1") == 0) printf("\nNome: %s\nEmail: %s\n\n", professor->nome, professor->email);
            ...
            carregar_usuarios(); Usuario *al = encontrar_usuario_por_email(email_aluno); if (!al) { printf("Aluno não encontrado.\n"); continue; }
            ...
            for (int i=0;i<n;i++) printf("%d - %s\n", i+1, lista[i]); char escolha[8]; printf("Escolha o número do curso para adicionar nota: "); if (!fgets(escolha, sizeof(escolha), stdin)) continue; trim(escolha); int idx = atoi(escolha); if (idx<1 || idx>n) { printf("Opção inválida.\n"); continue; }
